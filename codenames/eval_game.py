...
class GameRun:
    ...
    def __init__(self):
        ...
        parser.add_argument("--codemaster_training", help="With training on embedding", default=False)
        ...
        if sys.argv[1] != "human" or sys.argv[2] != "human":
            ...
            if args.codemaster_training:
                if args.glove_guesser is not None:
                    self.cm_kwargs["guesser_vecs"] = Game.load_glove_vecs(args.glove_guesser)
                    if args.glove_cm is not None:
                        self.cm_kwargs["master_vecs"] = Game.load_glove_vecs(args.glove_cm)
                    elif args.wordnet is not None:
                        self.cm_kwargs["master_vecs"] = Game.load_wordnet(args.wordnet)
                    elif args.w2v is not None:
                        self.cm_kwargs["master_vecs"] = Game.load_w2v(args.w2v)
                elif "guesser_w2v" in args.guesser:
                    self.cm_kwargs["guesser_vecs"] = Game.load_w2v(args.w2v)
                    if args.glove_cm is not None:
                        self.cm_kwargs["master_vecs"] = Game.load_glove_vecs(args.glove_cm)
                    elif args.wordnet is not None:
                        self.cm_kwargs["master_vecs"] = Game.load_wordnet(args.wordnet)
                        ...
                        self.cm_kwargs["master_vecs"] = Game.load_w2v(args.w2v)
                elif "guesser_wn" in args.guesser:
                    self.cm_kwargs["guesser_vecs"] = Game.load_wordnet(args.wordnet)
                    if args.glove_cm is not None:
                        self.cm_kwargs["master_vecs"] = Game.load_glove_vecs(args.glove_cm)
                    elif args.w2v is not None:
                        self.cm_kwargs["master_vecs"] = Game.load_w2v(args.w2v)
